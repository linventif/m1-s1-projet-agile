...
public class EntretienRepository extends UtilisateurRepository<Entretien> {
  ...
  public Entretien findByNomEntreprise(String nomEntreprise) {
    ...
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      return em.createQuery("SELECT e FROM Entretien e WHERE e.nomEntreprise = :nom",
          Entretien.class).setParameter("nom", nomEntreprise).getSingleResult();
    } catch (Exception e) {
      return null;
    }
  }
  ...
  public boolean existsByNomEntreprise(String nomEntreprise) {
    ...
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      Long count = em.createQuery("SELECT COUNT(e) FROM Entretien e WHERE e.nomEntreprise = :nom",
          Long.class).setParameter("nom", nomEntreprise).getSingleResult();
      return count > 0;
    } catch (Exception e) {
      return false;
    }
