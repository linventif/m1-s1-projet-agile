package fr.univ.m1.projetagile.controleTechnique.persistence;
...
public class ControleTechniqueRepository {
  ...
  public ControleTechnique findByVehiculeId(Long vehiculeId) {
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      TypedQuery<ControleTechnique> query =
          em.createQuery("SELECT ct FROM ControleTechnique ct WHERE ct.vehicule.id = :vehiculeId",
