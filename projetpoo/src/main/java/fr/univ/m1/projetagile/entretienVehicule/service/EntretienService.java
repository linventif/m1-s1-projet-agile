...
import fr.univ.m1.projetagile.entretienVehicule.persistence.EntretienRepository;

/**
 * Service layer for managing Entretien (maintenance company) operations. Extends UtilisateurService
 * to inherit common user operations such as authentication, email lookup, and password management
 * for maintenance company accounts.
 */
public class EntretienService extends UtilisateurService<Entretien, EntretienRepository> {
  ...
  public Entretien updateNomEntreprise(Entretien entretien, String nouveauNom) {
    ...
    if (repository.existsByNomEntreprise(nouveauNom)) {
      throw new IllegalArgumentException("Une entreprise avec ce nom existe déjà: " + nouveauNom);
    }
