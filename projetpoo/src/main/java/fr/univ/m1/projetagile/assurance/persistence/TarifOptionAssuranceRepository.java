...
public class TarifOptionAssuranceRepository {
  ...
  /**
   * Sauvegarde une entité {@link TarifOptionAssurance} en base de données. Si le tarif ne possède
   * pas encore d'identifiant, il est créé (persist), sinon il est mis à jour (merge) dans une
   * transaction.
   *
    ...
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      TypedQuery<TarifOptionAssurance> query =
          em.createQuery("SELECT t FROM TarifOptionAssurance t " + "JOIN FETCH t.grilleTarif "
              + "WHERE t.id = :id", TarifOptionAssurance.class);
  ...
  public List<TarifOptionAssurance> findByGrilleId(Long grilleId) {
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      TypedQuery<TarifOptionAssurance> query =
          em.createQuery("SELECT t FROM TarifOptionAssurance t " + "JOIN FETCH t.grilleTarif g "
              + "WHERE g.id = :grilleId " + "ORDER BY t.nomOption", TarifOptionAssurance.class);
