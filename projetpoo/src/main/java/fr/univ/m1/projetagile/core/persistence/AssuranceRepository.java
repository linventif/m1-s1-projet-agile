...
public class AssuranceRepository {
  ...
  /**
   * Sauvegarde une entité {@link Assurance} en base de données.
   * Si l'assurance ne possède pas encore d'identifiant, elle est créée (persist),
  ...
  public Assurance findById(Long id) {
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      TypedQuery<Assurance> query = em.createQuery(
          "SELECT a FROM Assurance a " + "JOIN FETCH a.grille " + "WHERE a.id = :id",
          Assurance.class);
  ...
  public List<Assurance> findAll() {
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      TypedQuery<Assurance> query = em.createQuery(
          "SELECT a FROM Assurance a " + "JOIN FETCH a.grille " + "ORDER BY a.nom",
          Assurance.class);
  ...
  public List<Assurance> findByGrilleId(Long grilleId) {
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      TypedQuery<Assurance> query = em.createQuery(
          "SELECT a FROM Assurance a " + "JOIN FETCH a.grille g " + "WHERE g.id = :grilleId "
              + "ORDER BY a.nom",
          Assurance.class);
