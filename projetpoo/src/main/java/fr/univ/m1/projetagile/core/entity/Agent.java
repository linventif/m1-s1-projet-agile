...
public abstract class Agent extends Utilisateur {
    ...
    public void modifierVehicule(Vehicule vehicule, String marque, String modele, String couleur, String ville, Double prix) {
        // Modifie les informations d'un véhicule
        if (vehicule != null && vehicules.contains(vehicule)) {
            if (marque != null) vehicule.setMarque(marque);
            if (modele != null) vehicule.setModele(modele);
            if (couleur != null) vehicule.setCouleur(couleur);
            if (ville != null) vehicule.setVille(ville);
            if (prix != null) vehicule.setPrixJ(prix);
        }
    }
