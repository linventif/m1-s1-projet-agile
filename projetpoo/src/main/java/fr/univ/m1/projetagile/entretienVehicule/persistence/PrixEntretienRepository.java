...
public class PrixEntretienRepository {
  ...
  public PrixEntretien findByEntretienAndVehiculeTypeAndModel(Entretien entretien, TypeV typeVehi,
    ...
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      return em
          .createQuery("SELECT p FROM PrixEntretien p WHERE p.entretien.idU = :entretienId "
              + "AND p.typeVehi = :type AND p.modeleVehi = :modele", PrixEntretien.class)
          .setParameter("entretienId", entretien.getIdU()).setParameter("type", typeVehi)
          .setParameter("modele", modeleVehi).getSingleResult();
    } catch (Exception e) {
      return null;
    }
  }
