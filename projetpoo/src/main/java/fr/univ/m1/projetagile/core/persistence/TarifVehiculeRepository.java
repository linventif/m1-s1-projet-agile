...
public class TarifVehiculeRepository {
  ...
  /**
   * Sauvegarde une entité {@link TarifVehicule} en base de données.
  ...
  public List<TarifVehicule> findByGrilleId(Long grilleId) {
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      TypedQuery<TarifVehicule> query =
          em.createQuery("SELECT t FROM TarifVehicule t " + "JOIN FETCH t.grilleTarif g "
  ...
  public TarifVehicule findByGrilleAndTypeAndModele(Long grilleId, TypeV type, String modele) {
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      TypedQuery<TarifVehicule> query =
          em.createQuery("SELECT t FROM TarifVehicule t " + "JOIN FETCH t.grilleTarif g "
              + "WHERE g.id = :grilleId " + "AND t.typeVehi = :type "
      ...
      throw new RuntimeException(
          "Erreur lors de la récupération du tarif véhicule pour type=" + type + ", modele="
              + modele,
          e);
    }
  }
  ...
  public List<TarifVehicule> findAll() {
    try (EntityManager em = DatabaseConnection.getEntityManager()) {
      TypedQuery<TarifVehicule> query =
          em.createQuery("SELECT t FROM TarifVehicule t " + "JOIN FETCH t.grilleTarif "
              + "ORDER BY t.typeVehi, t.modeleVehi", TarifVehicule.class);
      return query.getResultList();
    } catch (Exception e) {
    ...
    try {
      ...
      if (tarif == null) {
        throw new IllegalArgumentException(
            "Aucun tarif véhicule trouvé avec l'identifiant " + id);
      }
